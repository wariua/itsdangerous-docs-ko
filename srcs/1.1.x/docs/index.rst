...
안전하게 보내려면 어떻게 해야 할까? 요령은 바로 서명이다. 당신만
알고 있는 어떤 키가 있고, 데이터를 암호학적으로 서명해서 다른
누군가에게 넘긴다. 그 데이터를 돌려받았을 때 누가 데이터를
조작했는지 쉽게 확인할 수 있다.

따라서 수신자가 컨텐츠를 해독하고 그 안에 뭐가 있는지 볼 수는
있지만 당신의 비밀키를 가지고 있지 않는 한 컨텐츠를 변경할 수는
없다. 그러니 당신은 키를 안전하고 복잡하게 유지하기만 하면 된다.
